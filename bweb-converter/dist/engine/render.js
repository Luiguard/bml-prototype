...
export class BwebRenderer {
    ...
    _handleEvent(type, x, y, originalEvent) {
        // Z-Index Sorting für Hit-Testing (höchster z-index zuerst)
        const hitQueue = Array.from(this.nodes.values()).sort((a, b) => {
        ...
        for (const node of hitQueue) {
            ...
            if (x >= comp.x && x <= comp.x + comp.w && y >= comp.y && y <= comp.y + comp.h) {
                ...
                if (this.onEvent) {
                    this.onEvent(node.bdt.nodeId, type);
                } else if (type === 'click') {
    ...
    _drawNode(node) {
        ...
        if (blb.display === 4) return; // display: none
        ...
        if (node.bdt.text) {
            ...
            this.ctx.font = `${blb.fontWeight || 400} ${blb.fontSize || 16}px sans-serif`; // TODO: Font-Family Extraction
            ...
            // Custom Text-Wrapping für Canvas
            ...
            for (let n = 0; n < words.length; n++) {
                ...
                // Falls die Linie breiter als die Box ist, umbrechen
                if (testWidth > comp.w && n > 0) {
