...
const endIndex = contentJs.indexOf('} catch(e) {'); // This time use indexOf instead of lastIndexOf to avoid grabbing the catch block if we remove the try
let logicBlock = contentJs.substring(startIndex, endIndex);

// Find the first try { and remove it. The catch(e) is not included because we stopped BEFORE it!
