...
if (args[0] !== 'build' || !args[1] || !args[2]) {
    console.error("Usage: node step11-compiler.js build <input.html> <output.bweb>");
    process.exit(1);
}
...
const TAG_MAP = { '#text': 0, 'h1': 1, 'p': 2, 'div': 3, 'button': 4, 'h2': 5, 'img': 6, 'video': 7, 'header': 8, 'main': 9, 'article': 10 };
const NULL_ID = 0xFFFF;

(async () => {
    console.log(`Launching Puppeteer to extract layout, media, and semantics from ${inputPath}...`);
        ...
        function traverse(element, parentId = 0xFFFF) {
            ...
            if (isText) {
                ...
                for (const line of lines) {
                    ...
                    const nodeData = { id, parentId, tag: '#text', isText: true, textContent: line.text, layout, firstChild: 0xFFFF, nextSibling: 0xFFFF, flags: 0, a11y: null };
                    nodes.push(nodeData);
            ...
            for (let i = 0; i < attrs.length; i++) {
                const attr = attrs[i];
                if (attr.name.startsWith('aria-') || attr.name === 'role' || attr.name === 'alt' || attr.name === 'tabindex' || attr.name === 'id') {
            ...
            const nodeData = { id, parentId, tag, isText: false, textContent: src || '', layout, firstChild: 0xFFFF, nextSibling: 0xFFFF, flags: 0, a11y: Object.keys(a11y).length > 0 ? a11y : null };
            nodes.push(nodeData);
    ...
    console.log(`Extracted ${nodesData.length} nodes. Packing Media & A11y...`);

    // Handle Media (BIB & BVS)
    const images = [];
    const videos = [];
    let imgCounter = 0;
    let vidCounter = 0;
    ...
    for (const node of nodesData) {
        if (node.tag === 'img' && node.textContent) {
            ...
            if (fs.existsSync(filePath)) {
                ...
                images.push({ id: imgCounter, buffer });
                node.textContent = `bib://${imgCounter}`;
                imgCounter++;
            }
        }
        if (node.tag === 'video' && node.textContent) {
            const filePath = path.join(inputDir, node.textContent);
            if (fs.existsSync(filePath)) {
                const buffer = fs.readFileSync(filePath);
                videos.push({ id: vidCounter, buffer });
                node.textContent = `bvs://${vidCounter}`;
                vidCounter++;
            }
        }
    }
    ...
    // Build BIB Section
    let bibHeaders = Buffer.alloc(2 + (images.length * 10)); // count + (id, offset, length)
    ...
    // Build BVS Section
    let bvsHeaders = Buffer.alloc(2 + (videos.length * 10));
    bvsHeaders.writeUInt16BE(videos.length, 0);
    let bvsData = Buffer.alloc(0);
    hPos = 2;
    for (const vid of videos) {
        bvsHeaders.writeUInt16BE(vid.id, hPos);
        bvsHeaders.writeUInt32BE(bvsData.length, hPos + 2);
        bvsHeaders.writeUInt32BE(vid.buffer.length, hPos + 6);
        hPos += 10;
        bvsData = Buffer.concat([bvsData, vid.buffer]);
    }
    const bvsSection = Buffer.concat([bvsHeaders, bvsData]);

    // Build BAS Section
    const basBuffers = [];
    let a11yTextPool = Buffer.alloc(0);
    let basCount = 0;
    for (const node of nodesData) {
        if (node.a11y) {
            basCount++;
            const a11yBuf = Buffer.alloc(10);
            a11yBuf.writeUInt16BE(node.id, 0);
            const textBuf = Buffer.from(JSON.stringify(node.a11y), 'utf8');
            a11yBuf.writeUInt32BE(a11yTextPool.length, 2);
            a11yBuf.writeUInt32BE(textBuf.length, 6);
            basBuffers.push(a11yBuf);
            a11yTextPool = Buffer.concat([a11yTextPool, textBuf]);
        }
    }
    const basCountBuf = Buffer.alloc(2); basCountBuf.writeUInt16BE(basCount, 0);
    const basTextPoolSizeBuf = Buffer.alloc(4); basTextPoolSizeBuf.writeUInt32BE(a11yTextPool.length, 0);
    const basSection = Buffer.concat([basCountBuf, ...basBuffers, basTextPoolSizeBuf, a11yTextPool]);
    ...
    // Header: 6 sections (BDT=0, BML=1, BLB=2, BIB=4, BVS=5, BAS=6)
    const headerBuf = Buffer.alloc(6 + (6 * 9));
    headerBuf.write('BWEB', 0);
    headerBuf.writeUInt8(1, 4);
    headerBuf.writeUInt8(6, 5); // 6 sections
    ...
    headerBuf.writeUInt8(6, 51); headerBuf.writeUInt32BE(offset, 52); headerBuf.writeUInt32BE(basSection.length, 56);

    const bwebFile = Buffer.concat([headerBuf, bdtSection, bmlSection, blbSection, bibSection, bvsSection, basSection]);
