...
if (args[0] !== 'build' || !args[1] || !args[2]) {
    console.error("Usage: node step10-compiler.js build <input.html> <output.bweb>");
    process.exit(1);
}
...
const TAG_MAP = { '#text': 0, 'h1': 1, 'p': 2, 'div': 3, 'button': 4, 'h2': 5, 'img': 6, 'video': 7 };
const NULL_ID = 0xFFFF;

(async () => {
    console.log(`Launching Puppeteer to extract layout and media from ${inputPath}...`);
        ...
        function traverse(element, parentId = 0xFFFF) {
            ...
            if (isText) {
                ...
                for (const line of lines) {
                    ...
                    const nodeData = { id, parentId, tag: '#text', isText: true, textContent: line.text, layout, firstChild: 0xFFFF, nextSibling: 0xFFFF, flags: 0 };
                    nodes.push(nodeData);
            ...
            const nodeData = { id, parentId, tag, isText: false, textContent: src || '', layout, firstChild: 0xFFFF, nextSibling: 0xFFFF, flags: 0 };
            nodes.push(nodeData);
    ...
    console.log(`Extracted ${nodesData.length} nodes. Packing Media...`);
    ...
    // Header: 5 sections (BDT=0, BML=1, BLB=2, BIB=4, BVS=5)
    const headerBuf = Buffer.alloc(6 + (5 * 9));
    headerBuf.write('BWEB', 0);
    headerBuf.writeUInt8(1, 4);
    headerBuf.writeUInt8(5, 5); // 5 sections
    ...
    headerBuf.writeUInt8(5, 42); headerBuf.writeUInt32BE(offset, 43); headerBuf.writeUInt32BE(bvsSection.length, 47);

    const bwebFile = Buffer.concat([headerBuf, bdtSection, bmlSection, blbSection, bibSection, bvsSection]);
