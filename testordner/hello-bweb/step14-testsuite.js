...
const testsDir = path.join(__dirname, 'tests/edge-cases');
...
(async () => {
    ...
    for (const file of files) {
        ...
        await pageDOM.goto(`file://${htmlPath}`);
        await pageDOM.screenshot({ path: domScreenshotPath });
        await pageDOM.close();

        // 2. Compile BWEB
        execSync(`node step14-compiler.js build "${htmlPath}" "${bwebPath}"`, { stdio: 'ignore' });
        ...
        const viewerHtml = fs.readFileSync('step14-viewer.html', 'utf8').replace('dist/hello.bweb', `tests/output/${name}.bweb`);
