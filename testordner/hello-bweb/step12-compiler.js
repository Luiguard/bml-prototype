...
if (args[0] !== 'build' || !args[1] || !args[2]) {
    console.error("Usage: node step12-compiler.js build <input.html> <output.bweb>");
    process.exit(1);
}
...
const TAG_MAP = { '#text': 0, 'h1': 1, 'p': 2, 'div': 3, 'button': 4, 'h2': 5, 'img': 6, 'video': 7, 'header': 8, 'main': 9, 'article': 10, 'span': 11 };
const NULL_ID = 0xFFFF;

(async () => {
    console.log(`Launching Puppeteer to extract layout, media, semantics, and events from ${inputPath}...`);
        ...
        function traverse(element, parentId = 0xFFFF) {
            ...
            if (isText) {
                ...
                for (const line of lines) {
                    ...
                    const nodeData = { id, parentId, tag: '#text', isText: true, textContent: line.text, layout, firstChild: 0xFFFF, nextSibling: 0xFFFF, flags: 0, meta: null };
                    nodes.push(nodeData);
    ...
    console.log(`Extracted ${nodesData.length} nodes. Packing Media & Metadata...`);
    ...
    // Build BMS Section (Binary Metadata Section - replaced BAS)
    const bmsBuffers = [];
    ...
    // Header: 6 sections (BDT=0, BML=1, BLB=2, BIB=4, BVS=5, BMS=6)
