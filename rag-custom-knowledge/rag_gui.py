...
class RAGMainWindow(Gtk.Window):
    ...
    def _install_shortcut(self, *_):
        desktop_src = _BASE / "CustomRAG-Hauptmenue.desktop"
        if not desktop_src.exists():
            self._write_desktop_file(desktop_src)
        targets = [
            Path.home() / "Desktop" / "CustomRAG-Hauptmenue.desktop",
            Path.home() / ".local" / "share" / "applications" / "customrag-hauptmenue.desktop",
        ]
        msg = []
        for dest in targets:
            try:
    ...
    def _write_desktop_file(self, path: Path):
        ...
        path.write_text(
            f"""[Desktop Entry]
Type=Application
Name=CustomRAG Hauptmenü
GenericName=KI-Zentrale
Comment=Lokale Wissensbasis, Assistent und Domänen-Manager (ohne Terminal)
Exec={exec_path}
Path={_BASE}
Terminal=false
