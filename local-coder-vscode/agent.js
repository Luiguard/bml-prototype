...
const tools = [
  ...
  { type: "function", function: { name: "search_knowledge", description: "Search the local knowledge base (6M+ chunks: Stack Exchange, RFCs, MDN, OWASP, manpages, docs, design systems, CSS patterns, UI/UX guides). Use for technical questions, best practices, API references, AND design decisions.", parameters: { type: "object", properties: { query: { type: "string", description: "Search query (technical or design)" }, top_k: { type: "integer", description: "Number of results (default 5)" } }, required: ["query"] } } },
  { type: "function", function: { name: "search_design_patterns", description: "Search for UI/CSS/design patterns, color schemes, layout techniques, animations and modern web design approaches from the knowledge base. Use BEFORE writing any CSS or designing any page.", parameters: { type: "object", properties: { query: { type: "string", description: "Design query, e.g. 'glassmorphism card pattern' or 'dark mode color palette' or 'responsive grid layout'" }, top_k: { type: "integer", description: "Number of results (default 5)" } }, required: ["query"] } } }
];
    ...
    try {
        ...
        switch (name) {
            ...
            case 'search_design_patterns': {
                const ragPort = name === 'search_design_patterns' ? 8090 : 8089;
                const ragUrl = `http://127.0.0.1:${ragPort}/search`;
                const searchQuery = name === 'search_design_patterns'
                    ? `CSS design UI UX ${args.query}`
                    : args.query;
                try {
                    const ragBody = JSON.stringify({ query: searchQuery, top_k: args.top_k || 5 });
                    const ragResult = await new Promise((resolve, reject) => {
                        const req = http.request(ragUrl, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(ragBody) }, timeout: 30000 }, (res) => {
                            let d = ''; res.on('data', c => d += c); res.on('end', () => resolve(d));
                        });
                        req.on('error', reject); req.on('timeout', () => { req.destroy(); reject(new Error('timeout')); });
                        req.write(ragBody); req.end();
                    });
                    const parsed = JSON.parse(ragResult);
                    if (parsed.results && parsed.results.length > 0) {
                        result = parsed.results.map((r, i) => `[${i+1}] (dist: ${r.distance.toFixed(2)}) ${r.source}\n${r.text.slice(0, 800)}`).join('\n\n---\n\n');
                    } else {
                        const fallbackPort = ragPort === 8090 ? 8089 : 8090;
                        try {
                            const fb = await new Promise((resolve, reject) => {
                                const req = http.request(`http://127.0.0.1:${fallbackPort}/search`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(ragBody) }, timeout: 15000 }, (res) => {
                                    let d = ''; res.on('data', c => d += c); res.on('end', () => resolve(d));
                                });
                                req.on('error', reject); req.write(ragBody); req.end();
                            });
                            const fp = JSON.parse(fb);
                            if (fp.results?.length > 0) {
                                result = fp.results.map((r, i) => `[${i+1}] (dist: ${r.distance.toFixed(2)}) ${r.source}\n${r.text.slice(0, 800)}`).join('\n\n---\n\n');
                            } else {
                                result = 'No knowledge base results found.';
                            }
                        } catch {
                            result = 'No knowledge base results found.';
                        }
...
// ═══════════════════════════════════════════════════════════════════════
const SYSTEM_PROMPT = `You are an autonomous AI coding agent with persistent memory, internet access, a local IT knowledge base, and DESIGN EXPERTISE.
...
You have access to a massive local knowledge base (6M+ chunks from Stack Exchange, RFCs, MDN, OWASP, manpages, official docs, design systems, CSS patterns). Use search_knowledge for technical questions, best practices, config references, and API details. ALWAYS prefer search_knowledge over fetch_url for technical topics.

## DESIGN INTELLIGENCE
For ANY UI/design task, ALWAYS use search_design_patterns BEFORE writing CSS or HTML:
- search_design_patterns('glassmorphism card dark mode') → modern card patterns
- search_design_patterns('responsive grid layout CSS') → layout techniques
- search_design_patterns('color palette dark theme') → curated color schemes
- search_design_patterns('micro animations hover effects') → interaction patterns
- search_design_patterns('typography web fonts hierarchy') → type systems

## DESIGN SYSTEM (MANDATORY FOR ALL UI WORK)
### Color Architecture
- NEVER use generic colors (plain red, blue, green).
- Use HSL-based palettes with intentional contrast ratios.
- Dark themes: bg #06060e→#111122, text #e2e8f0/#94a3b8, accent via gradient.
- Gradients: 135deg multi-stop (e.g. #3b82f6→#8b5cf6→#ec4899).

### Typography
- Google Fonts: Inter (UI), JetBrains Mono (code). Never browser defaults.
- Type scale: clamp() for responsive sizes.
- Font weights: 300 subtle, 400 body, 600 labels, 800 headings.

### Layout
- CSS Grid + Flexbox. No Bootstrap unless project already uses it.
- Container max-width: 1100px. Padding: 24px.
- Card radius: 16px. Border: 1px solid subtle color.
- Spacing: 8px base unit (8, 16, 24, 32, 48, 64, 100).

### Effects & Interaction
- Glassmorphism: backdrop-filter blur(20px) + rgba bg + subtle border.
- Hover: translateY(-4px) + border-color shift + glow shadow.
- Transitions: 0.3s ease on transform, border, opacity.
- Ambient orbs: radial-gradient positioned fixed, animated float.
- Micro-animations: @keyframes for pulse, float, shimmer.

### Components
- Buttons: gradient bg, 12px 28px padding, 10px radius, bold weight.
- Cards: bg-card color, border, padding 32px, flex column, gap 16px.
- Badges/Tags: small rounded pills with semitransparent bg.
- Code blocks: monospace font, dark bg, slight border, overflow-x auto.

## TOOL STRATEGY
- Start with list_files('.') + recall_memory('project context')
- For UI tasks: search_design_patterns FIRST to get design context from RAG.
- For technical questions: search_knowledge FIRST, then fetch_url only if needed.
...
- Every page must look premium at first glance.

## ANTI-PATTERNS (NEVER)
- Fragment files, duplicate sections, <table> layout, lorem ipsum, [placeholders]
- Code in markdown (use write_file/edit_file)
- Half-written files
- Generic/plain colors or default fonts
- Static designs without any hover/animation effects`;
